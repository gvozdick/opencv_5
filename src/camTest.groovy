...
class camTest extends JFrame{
    ...
    public static void main(String[] args) {
        ...
        VideoCapture cap = new VideoCapture(1) //объект видеопотока
        Mat frame = new Mat()
...
//ищем контуры
            ...
            if (contours?.size() > 0) {
                for (int i = 0; i < contours.size(); i++) {
                    ...
                    Imgproc.drawContours(frame, contours, i, color, 1, Imgproc.LINE_8, hierarchy, 0, new Point());
               ...
               // println(maxArcIndex)
                ...
                for (int i = 0; i < approx.toArray().size(); i++) {
                    ...
                    Imgproc.circle(frame, points[i], 2, color1, 2)
                }

                //сal center of mass
                Moments moment = Imgproc.moments(approx)

                if(moment?.get_m00().toInteger()>100){
                    int x = 0
                    int y = 0
                    x = (moment?.get_m10() / moment?.get_m00()).toInteger()
                    y = (moment?.get_m01() / moment?.get_m00()).toInteger()
                ...
                //матрицы для warp
                    ...
                    Mat warped = new Mat()
                    Imgproc.warpPerspective(frame, warped, warpMat, new Size(a4Width, a4Length))
                    //масштабируем
                    ...
                    Imgproc.Canny(dilatated, cannyOutput, 80, 150)
                    ...
                    if (contours1?.size() > 0) {
                       ...
                       // println(maxArcIndex1)
                        ...
                        println("rotatedSize area = ${rotatedRect?.size.area()}")

                        if(maxArc>2000 && rotatedRect?.size.area()>20000){
                        ...
                        //вычисляем размеры объекта
                            ...
                            objW = ((vertices[0]?.x - vertices[1]?.x)**2 + (vertices[0]?.y - vertices[1]?.y)**2)**0.5
                            objW = objW * pixel
                            println("obj width = ${objW}")
                            objL = ((vertices[0]?.x - vertices[3]?.x)**2 + (vertices[0]?.y - vertices[3]?.y)**2)**0.5
                            objL = objL * pixel
                        ...
                        Imgcodecs.imencode(".png", frame, buf) //преобразуем кадр в формат png и записываем в buf
